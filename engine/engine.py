...
# Load .obj with triangulation and caching
def load_obj(path, scale=1.0, offset=(0, 0, 0), cache_dir="cache"):
    """Load OBJ file or its cached binary version."""
    import os
    cache_file = os.path.join(cache_dir, os.path.basename(path) + ".npz")
    
    # Check if cached binary exists
    if os.path.exists(cache_file):
        print(f"Loading cached data from {cache_file}")
        data = np.load(cache_file)
        verts = data['verts'].tolist()
        ...
        np.savez(cache_file, verts=verts, faces=faces)
